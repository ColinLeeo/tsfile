    ...
    for (const auto& range : ranges) {
        ...
        for (;;) {
            ...
            if (!(result->get_value<int32_t>(3) == v.i32 &&
                  ...
                  result->get_value<int32_t>(5) == v.i32_2 &&
                  result->get_value<int64_t>(6) == v.i64_2 &&
                  result->get_value<int64_t>(7) == v.i64_3)) {
                std::cerr << std::setprecision(17)
                          << "MISMATCH device=" << device << " row=" << row
                          << " actual=" << result->get_value<int32_t>(3) << ","
                          << result->get_value<int64_t>(4) << ","
                          << result->get_value<int32_t>(5) << ","
                          << result->get_value<int64_t>(6) << ","
                          << result->get_value<int64_t>(7)
                          << " expected=" << v.i32 << "," << v.i64 << ","
                          << v.i32_2 << "," << v.i64_2 << "," << v.i64_3
                          << std::endl;
...
void memory_sample(TsFileWriter* writer, const char* phase, uint64_t rows,
                   Clock::time_point start) {
    struct rusage usage = {};
    require(getrusage(RUSAGE_SELF, &usage) == 0, "memory getrusage failed");
    task_vm_info_data_t vm = {};
    mach_msg_type_number_t count = TASK_VM_INFO_COUNT;
    require(task_info(mach_task_self(), TASK_VM_INFO,
                     reinterpret_cast<task_info_t>(&vm), &count) == KERN_SUCCESS,
            "memory task_info failed");
    malloc_statistics_t heap = {};
    malloc_zone_statistics(nullptr, &heap);
    std::cout << "{\"event\":\"memory\",\"phase\":\"" << phase
              << "\",\"rows\":" << rows
              << ",\"elapsed_s\":" << seconds(start, Clock::now())
              << ",\"rss_bytes\":" << vm.resident_size
              << ",\"peak_rss_bytes\":" << usage.ru_maxrss
              << ",\"physical_footprint_bytes\":" << vm.phys_footprint
              << ",\"reusable_bytes\":" << vm.reusable
              << ",\"internal_bytes\":" << vm.internal
              << ",\"external_bytes\":" << vm.external
              << ",\"physical_footprint_peak_bytes\":" << vm.ledger_phys_footprint_peak
              << ",\"compressed_bytes\":" << vm.compressed
              << ",\"heap_in_use_bytes\":" << heap.size_in_use
              << ",\"heap_reserved_bytes\":" << heap.size_allocated
              << ",\"writer_chunk_estimate_bytes\":"
              << (writer ? writer->calculate_mem_size_for_all_group() : -1)
              << ",\"writer_metadata_estimate_bytes\":"
              << (writer ? writer->calculate_meta_mem_size() : -1)
              << "}" << std::endl;
}
...
int main(int argc, char** argv) {
    try {
        ...
        memory_sample(nullptr, "startup", 0, start);
        ...
        std::vector<int32_t> i32_2(kTabletRows);
        std::vector<int64_t> i64_2(kTabletRows);
        ...
        for (uint32_t c = 0; c < kNames.size(); ++c) {
            const TSEncoding encoding =
                c == 0 || o.codec != "default_lz4" ? PLAIN : TS_2DIFF;
        ...
        memory_sample(&writer, "setup", 0, start);
        uint64_t tablets = 0;
        ...
        for (uint64_t base = 0; base < o.rows; base += kDeviceRows) {
            ...
            check(tablet.set_column_values(2, i64.data(), nullptr, kTabletRows),
                  "bulk i64");
            check(
                tablet.set_column_values(3, i32_2.data(), nullptr, kTabletRows),
                "bulk i32_2");
            check(
                tablet.set_column_values(4, i64_2.data(), nullptr, kTabletRows),
                "bulk i64_2");
            check(
                tablet.set_column_values(5, i64_3.data(), nullptr, kTabletRows),
                "bulk i64_3");
            ...
            ++tablets;
            if (tablets % 10 == 0) memory_sample(&writer, "write", tablets * kTabletRows, start);
        ...
        memory_sample(&writer, "before_final_flush", tablets * kTabletRows, start);
        const auto flush_start = Clock::now();
        check(writer.flush(), "writer.flush");
        memory_sample(&writer, "after_final_flush", tablets * kTabletRows, start);
        ...
        memory_sample(nullptr, "after_close", tablets * kTabletRows, start);
        malloc_zone_print(nullptr, false);
        const size_t released = malloc_zone_pressure_relief(nullptr, 0);
        std::cout << "{\"event\":\"allocator_relief\",\"released_bytes\":" << released << "}" << std::endl;
        memory_sample(nullptr, "after_allocator_relief", tablets * kTabletRows, start);
