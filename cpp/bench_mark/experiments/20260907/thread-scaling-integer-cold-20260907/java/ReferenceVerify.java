  ...
  public static void main(String[] args) throws Exception {
    ...
    try(ITsFileReader reader=new TsFileReaderBuilder().file(new File(args[0])).build()) {
      for(long[] range:new long[][]{{0,2},{9998,10002},{ROWS/2,ROWS/2+2},{ROWS-10000,ROWS-1}}) {
        ...
        try(ResultSet rs=reader.query("benchmark",Arrays.asList("device","i32","i64","i32_2","i64_2","i64_3"),EPOCH+range[0],EPOCH+range[1])) {
          while(rs.next()) {
            ...
            check(rs.getInt(5)==((int)((a>>>32)&0xffffff)-0x800000),"i32_2 "+d+":"+row);
            check(rs.getLong(6)==(b>>>12),"i64_2 "+d+":"+row);
            check(rs.getLong(7)==(b&0x7fffffffffffffffL),"i64_3 "+d+":"+row+" actual="+rs.getLong(7)+" expected="+(b&0x7fffffffffffffffL));
            checked++;
          }
        }
